...
export default function SearchDropdown({
  ...
  const optOptions = () => {
    if (filteredOptions?.length == 0) {
      return (
        <li className={"text-2xl p-2 text-[#979797] bg-[#1f1f1f]"}>
          Não encontrado
        </li>
      );
    }
    ...
    if (opt == 1) {
      return filteredOptions?.map((opt, index) => (
        <li
          ...
          className={
            "text-2xl p-2 text-ellipsis pl-4 bg-[#1f1f1f] transition-[1s] hover:cursor-pointer hover:bg-[#353535]"
          }
        >
          {opt.MaqCodigo}
        </li>
      ...
      return filteredOptions?.map((opt, index) => (
        <li
          ...
          className={
            "text-2xl p-2  pl-4 bg-[#1f1f1f] transition-[1s] hover:cursor-pointer hover:bg-[#353535]"
          }
        >
          {opt.SntDescricao}
        </li>
      ...
      return filteredOptions?.map((opt, index) => (
        <li
          key={index}
          onMouseDown={() => handleOptionSelect([opt.ManNome, opt.ManCod])}
          className={
            "text-2xl p-2  pl-4 bg-[#1f1f1f] transition-[1s] hover:cursor-pointer hover:bg-[#353535]"
          }
        >
  ...
  return (
    <div className="dropdown">
      ...
      <input
        ...
        value={searchText}
        className="p-3 pr-10 text-2xl bg-input focus:outline-1 focus:outline-[#fff] focus:-outline-offset-2 w-full z-100 border-[2px] rounded-md hover:cursor-pointer placeholder-shown:text-ellipsis"
      ...
      <div className="relative  z-50 ">
        <ul
          className={
            "w-full border-x-[4px] max-h-[15em] shadow overflow-y-auto border-b-[4px] absolute rounded-b-md pt-2 -mt-2 " +
            ulstyle
          }
        >
          {optOptions()}
