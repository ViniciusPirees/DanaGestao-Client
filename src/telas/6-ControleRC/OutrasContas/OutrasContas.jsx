...
import FiltrosOutras from "./FiltrosOutras";
...
export default function OutrasContas() {
  ...
  const filtrosSalvos =
    localstorage?.filtrosControleOutras == undefined
      ? [
          [
      ...
      : JSON.parse(localstorage?.filtrosControleOutras);

  const orderValSalvos =
    localstorage?.orderValControleOutras == undefined
      ? "desc"
      : localstorage?.orderValControleOutras;

  const orderSalvos =
    localstorage?.orderControleOutras == undefined
      ? "CREATION_DATE"
      : localstorage?.orderControleOutras;
  ...
  const qtdPagSalvoOutCon =
    localstorage?.qtdPagSalvoOutCon == undefined
      ? "10"
      : localstorage?.qtdPagSalvoOutCon;

  const [selecQtdPag, setSelecQtdPag] = useState(qtdPagSalvoOutCon);
  ...
  //GET ControleOutras POR DESCRICAO
  const getRCS = async () => {
    setLoad2(true);
    localStorage.setItem("filtrosControleOutras", JSON.stringify(filtroAll));
    localStorage.setItem("orderControleOutras", order);
    localStorage.setItem("orderValControleOutras", orderVal);
    localStorage.setItem("qtdPagSalvoOutCon", selecQtdPag);
    getControleRCCount({
      filtroAll,
      tabela: "ConsultaOutras",
    });
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full pl-[8.5em] pr-10 laptop:pl-[6.5em] tablet:pl-[6.5em] ">
        ...
        <Titulo titulo="Controle RC (311212 - 311217 - 311258)" />
        <div className="">
