...
export default function ControleStatus() {
  ...
  const filtrosSalvos =
    localstorage?.filtrosControleStatus == undefined
      ? [
          [
      ...
      : JSON.parse(localstorage?.filtrosControleStatus);

  const orderValSalvos =
    localstorage?.orderValControleStatus == undefined
      ? "desc"
      : localstorage?.orderValControleStatus;

  const orderSalvos =
    localstorage?.orderControleStatus == undefined
      ? "CREATION_DATE"
      : localstorage?.orderControleStatus;
  ...
  const qtdPagSalvoConSta =
    localstorage?.qtdPagSalvoConSta == undefined
      ? "10"
      : localstorage?.qtdPagSalvoConSta;

  const [selecQtdPag, setSelecQtdPag] = useState(qtdPagSalvoConSta);
  ...
  const getRCS = async () => {
    setrcs([]);
    setLoad2(true);
    localStorage.setItem("filtrosControleStatus", JSON.stringify(filtroAll));
    localStorage.setItem("orderControleStatus", order);
    localStorage.setItem("orderValControleStatus", orderVal);
    localStorage.setItem("qtdPagSalvoConSta", selecQtdPag);
    getControleRCCount({
      filtroAll,
      tabela: "ControleStatus",
      status: 1,
    });
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full pl-[8.5em] pr-10 laptop:pl-[6.5em] tablet:pl-[6.5em] ">
        ...
        <Titulo titulo="Controle Status (311250 - 311249)" />
        <div className="">
