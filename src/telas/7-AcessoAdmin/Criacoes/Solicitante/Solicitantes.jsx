...
import FiltroSol from "./FiltroSol";
...
export default function Solicitantes() {
  const [filtro, setFiltro] = useState("SntDescricao");
  ...
  useEffect(() => {
    ...
    changePage({ selected: 0 }).then(getSolicitantes([input, filtro]));
    setSucesso(false);
  }, [selecQtdPag, sucesso]);

  useEffect(() => {
    getSolicitantes([input, filtro]);
  }, [pageN]);

  const getSolicitantes = async (props) => {
    getManCount([props[1], props[0], "Solicitantes"]);
    try {
      ...
      console.log(err);
      Notificacao(["erro", "Erro ao buscar os Solicitantes" + err]);
    }
  };
  ...
  const renderTableData = () => {
    return Solicitantes?.map((Solicitante, index) => (
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full ml-[8.5em] mr-10 h-full overflow-y-auto">
        ...
        <Titulo titulo="Cadastros - Solicitante" />
        <div>
          <div>
              ...
              <table className="table table-auto rounded-lg mx-auto mb-20 w-full">
                <thead>
                  <tr className="text-2xl bg-dana font-extrabold">
                    ...
                    <th className="p-3 w-">Descrição do Solicitante</th>
                    <th className="w-0"></th>
                    <th className="w-0"></th>
                  </tr>
                </thead>
