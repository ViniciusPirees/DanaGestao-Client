...
import FiltroForn from "./FiltroForn";
...
export default function Fornecedor() {
  const [filtro, setFiltro] = useState("ForDes");
  ...
  useEffect(() => {
    ...
    changePage({ selected: 0 }).then(getFornecedores([input, filtro]));
    setSucesso(false);
  }, [selecQtdPag, sucesso]);

  useEffect(() => {
    getFornecedores([input, filtro]);
  }, [pageN]);

  const getFornecedores = async (props) => {
    ...
    try {
      ...
      console.log(err);
      Notificacao(["erro", "Erro ao buscar os Fornecedores" + err]);
    }
  };
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full ml-[8.5em] mr-10 h-full overflow-y-auto">
        ...
        <Titulo titulo="Cadastros - Fornecedor" />
        <div>
          <div>
