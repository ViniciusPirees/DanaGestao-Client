...
import FiltroTipoStatus from "./FiltroTipoStatus";
...
export default function TipoStatus() {
  const [filtro, setFiltro] = useState("TipStaDesc");
  const [tipo, setTipo] = useState(0);
  const [tiposSta, settiposSta] = useState([]);
  ...
  useEffect(() => {
    ...
    changePage({ selected: 0 }).then(gettiposSta([input, filtro]));
    setSucesso(false);
  }, [selecQtdPag, sucesso]);

  useEffect(() => {
    gettiposSta([input, filtro]);
  }, [pageN]);

  const gettiposSta = async (props) => {
    getManCount([props[1], props[0], "TipoStatus"]);
    try {
      ...
      console.log(err);
      Notificacao(["erro", "Erro ao buscar as Tipo Status" + err]);
    }
  };
  ...
  const renderTableData = () => {
    return tiposSta?.map((tipoSta, index) => (
     <tr
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full ml-[8.5em] mr-10 h-full overflow-y-auto">
        ...
        <Titulo titulo="Cadastros - Tipo Status" />
        <div>
          <div>
              ...
              <table className="table table-auto tablet:table-fixed rounded-lg mx-auto  w-full">
                <thead>
                  <tr className="text-xl tablet:text-lg  bg-dana font-extrabold">
                    ...
                    <th className="p-3 w-">Descrição da Tipo Status</th>
